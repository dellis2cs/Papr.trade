  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("http://localhost:8000/users/login", {
        ...
        // Ensure the backend expects the right field names.
        // If your backend login is based on username, you might need to adjust here.
      ...
      if (!res.ok) {
        ...
        // Store user_id (and potentially a token) for later use
        localStorage.setItem("user_id", data.user_id)
        // Navigate to a protected route (for example, /dashboard)
        navigate("/dashboard")
      }
    } catch (err: any) {
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-6">
        ...
        <div className="grid gap-2">
          <div className="flex items-center">
            ...
            <a href="#" className="ml-auto text-sm text-[#526fff] hover:underline">
      ...
      <div className="text-center text-sm text-white/60">
        ...
        <Link to="/SignUp">
          <h1 className="text-[#526fff] hover:underline">Sign up</h1>
        </Link>
      </div>
    </motion.form>
  )
